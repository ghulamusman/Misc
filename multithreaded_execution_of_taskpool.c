...
void init(long numberThreads, int qsize) {
	...
	queue->fn = malloc(sizeof( void (**) (void) )*queue->qLength); 
	for(int i = 0; i<queue->qLength; i++){
		queue->fn[i] = NULL;
	}
	...
	// execute emptyTask()
	poolStruct->pool = malloc(poolStruct->count*sizeof(pthread_t));
	for (thread = 0; thread < poolStruct->count; thread++)
      	pthread_create(&poolStruct->pool[thread], NULL,emptyTask, (void*) thread);
